...
def weighted_dash(bitrates, dash_player, weighted_dwn_rate, curr_bitrate,
                  next_segment_sizes):
    """
    ...
    delay = 0
    next_bitrate = None
    available_video_segments = dash_player.buffer.qsize(
    ...
    config_dash.LOG.debug(
        ...
        "Beta = {} WDR = {}, curr Rate = {}".format(
            dash_player.buffer.qsize(), dash_player.initial_buffer,
            available_video_duration, dash_player.alpha, dash_player.beta,
            weighted_dwn_rate, curr_bitrate))
    ...
    # switch to a lower suitable bitrate
        ...
        if curr_bitrate >= max(bitrates):
            config_dash.LOG.info(
                "Current bitrate is MAX = {}".format(curr_bitrate))
            ...
            config_dash.LOG.info(
                ...
                "available_video={} seconds, ratio = {}".format(
                    ...
                    weighted_dwn_rate))
            if float(next_segment_sizes[higher_bitrate]
                     ) / weighted_dwn_rate < available_video_duration:
        ...
        next_bitrate = curr_bitrate
    config_dash.LOG.debug(
        "The next_bitrate is assigned as {}".format(next_bitrate))
    return next_bitrate, delay
